$(document).ready(function () {
    ...
    layui.use(['table', 'layer', 'flow'], function () {
        ...
        function loadNore(type, reverse) {
            ...
            $.ajax({
                type: "GET",
                url: '/strategy/public_sentiment_ajax/',
                data: {},
                success: function (data) {
                    ...
                    if (resultAll.length == 0) {
                        ...
                        for (var i = 0; i < resultAll.length; i++) {
                            resultAll[i].stock_drop = parseFloat(resultAll[i].stock_drop);
                            ...
                            for (var j = 0; j < resultAll[i].event.length; j++) {
                                option += '<tr>\n' +
                                    ...
                                    '        <td title="共出现次数: ' + resultAll[i].event[j].event_count.all_count + ' 影响次数: ' + resultAll[i].event[j].event_count.drop_count + '">' + resultAll[i].event[j].corr.toFixed(2) + '%</td>\n' +
                                    '    </tr>';
                            }
